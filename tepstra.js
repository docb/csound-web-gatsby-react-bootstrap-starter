...
import { Stage, Layer, Group, Text, Shape } from 'react-konva';

class Tepstra extends React.Component {
  constructor(props) {
    super(props);
    this.size=props.def.size;
    this.props.def.setMode = this.setMode.bind(this);
  ...
  createKeys = () => {
    ...
    for(let x=1;x<=20;x++) {
        ...
        for(let y=10;y>0;y--) {
            ...
            let untouchedColor;
            switch(note) {
            case 0: case 2: case 4: case 5: case 7: case 9: case 11: untouchedColor = 'white'; break;
            default: untouchedColor = 'gray';
            }
            let posX = x*(size*1.5);
            let posY;
            if(x%2==0) posY = (Math.sqrt(3)/2)*y*size*2; else posY = (Math.sqrt(3)/2)*(y*size*2-size);
            let oct = Math.floor(ynr/12);
            let id = 'id-'+x+'-'+y;
            keys[id]={x:posX,y:posY,size:size, note:currentNr,
